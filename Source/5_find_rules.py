...
def run_spmf_rules(var_name, algo, arg):
  
    input_file = LOCATION_RESULTS+var_name+"_input_spmf.txt"
    arguments = arg
    spmf = Spmf(algo, input_filename=input_file,
            output_filename = LOCATION_RESULTS+var_name+"_rules_output.txt", 
            spmf_bin_location_dir="/home/michelav/.local/lib/python3.8/site-packages/spmf/",
            arguments=arguments)
    spmf.run()    
...
def find_rules(formatted_data, dict_hierc, W, algo, arg):
    ...
    for k, l in tqdm(dict_hierc.items()):
        ...
        to_transaction_db(data_tmp, var_name=k, W=W)
        run_spmf_rules(var_name=k, algo=algo, arg=arg)
...
def get_rules(data_cluster, dict_hierc, W):
    
    st = time.time()
    find_rules(data_cluster, dict_hierc, W, ALGOS[1], ARGSS[1])
    elapsed_time = time.time() - st
    print('Execution time:', elapsed_time, 's')
...
# Find patterns 
ALGOS           = ['PrefixSpan', 'RuleGrowth'] 
MIN_COV_PERC    = .05
MIN_CONF_PERC   = .1
LEN_PATTS       = 3
ARGSS           = [[MIN_COV_PERC, LEN_PATTS], [.01, .1, 3, 1]]
...
get_rules(disc_data_alarms, DICT_HIERC, W)

NAMES_TO_IDXS.to_csv('data/NAMES_TO_IDXS_rules.csv')